...
const Users = () => {
  ...
  const formatDate = (timeStamp) => {
    ...
    const formmattedTime = `${String(date.getHours()).padStart(2, '0')}:${String(date.getMinutes()).padStart(2, '0')}:${String(date.getSeconds()).padStart(2, '0')}`;

    const result = `${formattedDate}  ${formmattedTime}`;
    return result;
  }

  // const result = formatDate("2025-09-29T20:09:40.378+00:00");
  // console.log(result);
              ...
              <tbody>
                {users.filter((u) => u.role === "User").map((user, index) => (
                  <tr key={user._id} className={(index + 1) % 2 === 0 ? "bg-gray-50" : ""}>
                    ...
                    <td className="px-4 py-2">{user?.borrowedBooks.lenth}</td>
                    <td className="px-4 py-2">{user.role}</td>
